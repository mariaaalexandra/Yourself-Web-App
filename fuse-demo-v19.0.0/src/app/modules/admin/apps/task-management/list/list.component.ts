...
export class TasksListComponent implements OnInit, OnDestroy
{
    ...
    onClick(id:number){
        ...
        localStorage.setItem("taskId",String(id));
    }
    ...
    saveTask(type: 'task' | 'section'): void {
        const task = new Task()
        task.type = type;
        task.userId =[this.userId]
        // Populate the task object with data from your form or UI elements

        // Add other fields as necessary

        this.taskService.saveTask(task).subscribe({
          next: (savedTask) => {
            this._router.navigate(['./', savedTask.id], {relativeTo: this._activatedRoute});

            // Mark for check
            this._changeDetectorRef.markForCheck();
            localStorage.setItem("taskId",String(savedTask.id));
            console.log('Task saved successfully', savedTask);
            // Handle successful save, e.g., by redirecting the user or clearing the form
          },
          error: (error) => {
            console.error('Error saving task:', error);
            // Handle save error, e.g., by displaying a user-friendly message
          },
        });
      }
