...
import { Tag } from 'app/modules/admin/apps/task-management/tasks.types';
...
export class TasksService
{
     ...
     * Constructor
     */
    constructor(private _httpClient: HttpClient, private taskManagement: TaskManagementService)
    {
    }
    ...
    getTaskById(id: number): Observable<Task> {
        return this.taskManagement.findById(+id) // Convert `id` to number if necessary
          .pipe(
            map((task) => {
              if (!task) {
                throw new Error('Task not found'); // If backend returns null/undefined, throw an error
              }
              console.log("pla " + JSON.stringify(task))
              this._task.next(task); // Update the BehaviorSubject
              return task;
            }),
            catchError((error) => {
              // Handle errors appropriately
              console.error('Error fetching task by ID:', error);
              return throwError(() => new Error('Could not find task with id of ' + id + '!'));
            })
          );
      }
    /**
