...
export class InventoryService
{
    ...
    {
        return this._httpClient.get< InventoryProduct[]>('http://localhost:8080/api/products', {
            // params: {
            //     page: '' + page,
            //     size: '' + size,
            //     sort,
    ...
    createProduct(product: InventoryProduct): Observable<InventoryProduct> {
        ...
        return this.products$.pipe(
            take(1),
            switchMap(products =>
                // Directly use the HTTP POST request logic from addProduct here
                this._httpClient.post<InventoryProduct>('http://localhost:8080/api/products', product).pipe(
                    map((newProduct) => {
    ...
    updateProduct(id: string, product: InventoryProduct): Observable<InventoryProduct> {
        ...
        return this.products$.pipe(
            take(1),
            switchMap(products => this._httpClient.put<InventoryProduct>(`${baseUrl}`, product, { params }).pipe(
                map((updatedProduct) => {
    ...
    deleteProduct(id: string): Observable<boolean>
    {
        return this.products$.pipe(
            ...
            switchMap(products => {
                ...
                return this._httpClient.delete('http://localhost:8080/api/products/delete', {params: params}).pipe(
    ...
    {
        return this._httpClient.get<InventoryTag[]>('http://localhost:8080/api/products/getTags').pipe(
            tap((tags) =>
            {
    ...
    createTag(tag: InventoryTag): Observable<InventoryTag> {
        return this.tags$.pipe(
            take(1), // Take the current state of tags
            switchMap(tags =>
                this._httpClient.post<InventoryTag>('http://localhost:8080/api/products/addTag', tag) // Note the URL and request body
                .pipe(
                    map((newTag) => {
                        // Update the tags with the new tag by spreading the current tags and adding the new one
                        this._tags.next([...tags, newTag]);

                        // Return the new tag from the observable
                        return newTag;
                    }),
                )
            ),
